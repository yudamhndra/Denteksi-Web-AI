...
<div class="card">
    <div class="card-body">
        <div class="card-title">
            <h4 class="mb-0">Edit Sesi Klinik</h4>
        </div>
        <hr />
        <div class="panel-body">
            <form class="form-horizontal form-validate-jquery" action="{{route('sesi.update', $jadwalSesi->id)}}" method="post" enctype="multipart/form-data" files=true>
                @csrf
                @method('PUT')
                <fieldset class="content-group">
                    <div class="form-group mb-3">
                        ...
                        <select class="form-select" name="sesi">
                            ...
                            <option value="{{$value}}" {{
                                $jadwalSesi->sesi === $value ? 'selected' : ''
                            }}>{{$value}}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('sesi'))
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="text" name="kode_poli" class="form-control" value="{{$jadwalSesi->poliHari->poli->kode}} - {{$jadwalSesi->poliHari->poli->poli}}" disabled>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-select" name="mulai">
                            ...
                            <option value="{{$value['mulai']}}" {{
                                $jadwalSesi->mulai == $value['mulai'] ? 'selected' : ''
                            }}>{{$value['mulai']}}</option>
                            @endforeach
                        </select>
                        @if ($errors->has('mulai'))
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-select" name="selesai">
                            ...
                            <option value="{{$value['selesai']}}" {{
                                $jadwalSesi->selesai == $value['selesai'] ? 'selected' : ''
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="number" name="jumlah_dokter" class="form-control" value="{{$jadwalSesi->jumlah_dokter}}" placeholder="Jumlah dokter">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select id="dokter" name="dokter[]" class="form-control multi-select" multiple="multiple">
                            ...
                            <option value="{{$value->id}}" {{
                                in_array($value->id, $jadwalSesi->dokterSesi->pluck('dokter_id')->toArray()) ? 'selected' : ''
                            }}>{{$value->nama}}</option>
                            @endforeach
                        </select>
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{route('klinik.sesi.index', $jadwalSesi->poliHari->poli_id)}}"type="button" class="col-auto btn btn-secondary me-3"> <i class="icon-arrow-left13"></i> Kembali</a>
